...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old: 
        ...
        new_labels = [replace_leaf(branch, old, new) for branch in branches(t)]
        return tree(label(t), new_labels)
...
def move_stack(n, start, end):
    ...
    "*** YOUR CODE HERE ***"
    def moves(n, first, second, third):
        if n == 1:
            print_move(first, third)
        else: 
            moves(n-1, first, third, second)
            print_move(first, third)
            moves(n-1, second, first, third)
    return moves(n, start, start+1, end)
...
def balanced(m):
    ...
    "*** YOUR CODE HERE ***"
    results = []
    left, right = branches(sides(m)[0])[0], branches(sides(m)[1])[0] 
    left_length, right_length = length(sides(m)[0]), length(sides(m)[1]) 
    left_weight, right_weight = total_weight(left), total_weight(right)
    results += [left_weight*left_length == right_weight*right_length]
    if not is_weight(left):
        results += [balanced(left)]
    if not is_weight(right):
        results += [balanced(right)]
